...
1. `matcher::normalize`:
   ...
   - Returns `Result<Normalized, NormalizeError>` where `Normalized { text: String, offset_map: Vec<u32> }` and `offset_map[i]` is the starting byte offset in the original text for normalized-byte `i`. Single-pass.
   - Reject on length: normalized > `MAX_NORMALIZED_BYTES` (192 KiB) → `NormalizeError::TooLarge`. `Engine::scan` calls `normalize` internally and forwards the error to its caller, which translates to 413 at the HTTP boundary.
...
3. `matcher::scan`:
   ...
   - `Engine::scan(text: &str, langs: &[Lang], mode: Option<Mode>) -> Result<ScanResult, NormalizeError>` where `ScanResult { mode_used, matches, truncated }`. `Engine::scan` invokes `matcher::normalize` internally on the raw text; a `NormalizeError::TooLarge` surfaces as the `Err` branch so the HTTP handler can translate to 413. `mode = Some(m)` applies `m` uniformly to every scanned language (including CJK — no clamping) and echoes `m` in `mode_used` for each; `mode = None` looks each lang up in the `DEFAULT_MODE` table (populated in M3) and echoes the resolved value. `mode_used` always has one entry per scanned language.
...
4. Tag `v1.0.0`; cut image `ghcr.io/.../vocab-veto:v1.0.0` and `:$LIST_SHA` — once CLI CM5 lands, `.github/workflows/release.yml` automates both this and the `vv` musl binary upload on tag push.
